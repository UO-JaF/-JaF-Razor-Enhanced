...
# Original V1.0: 03/04/2023 Current V1.2 03/09/2023
#
...
cat = 0x00C9
rabbit = 0x00CD
Misc.IgnoreObject(cat)
Misc.IgnoreObject(rabbit)
def attack():
    while not Player.IsGhost:
        ...
        while enemy != None and not Player.IsGhost: 
            ...
            Misc.Pause(100)
            Mobiles.Message(enemy, 5, "The Color Is: {:08x} ".format(enemy.Hue))
            Misc.Pause(100)
            Timer.Create("longkill", 60000)
            while Mobiles.FindBySerial(enemy.Serial) and not Player.IsGhost: 
            ...
            while not Mobiles.FindBySerial(enemy.Serial) and not Player.IsGhost:
